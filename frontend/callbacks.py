...
def register_callbacks(app):
        ...
        def no_data(title):
            ...
            f.update_layout(
                ...
                hoverlabel=dict(
                    ...
                    font=dict(family="Fira Sans, sans-serif", size=13, color="#F4F4F5"),
                ),
            )
            return f
    ...
    def render_event_header(event_id, site_sel):
        ...
        def _safe_indicator(title, val, suffix="%", vmax=100):
            ...
            if v >= 70:
                bar_color = "#EF4444"  # Danger red
            elif v >= 50:
                bar_color = "#F59E0B"  # Warning amber
            elif v >= 30:
                bar_color = "#3B82F6"  # Primary blue
            else:
                bar_color = "#22C55E"  # Success green
                ...
                go.Indicator(
                    ...
                    gauge={
                        ...
                        "steps": [
                            ...
                            {"range": [30, 50], "color": "rgba(59, 130, 246, 0.1)"},
            ...
            try:
                ...
                fig_conf_evo.update_layout(
                    ...
                    hoverlabel=dict(
                        ...
                        bordercolor="rgba(255, 255, 255, 0.1)",
                        font=dict(
                            family="Fira Sans, sans-serif", size=13, color="#F4F4F5"
                        ),
                    ),
                )
        else:
    ...
    def open_action_modal(n_clicks_list, button_ids):
        ...
        # Build context with all data needed for pattern recording
        context = {
            "site_id": site_id,
            "event_id": event_id,
            "action": action,
            ...
            "severity": inc.get("severity_max", "S1"),
            # Additional data for pattern recording
            "subscores_ui": inc.get("subscores_ui", {}),
            "subscores": inc.get("subscores", {}),
            "mnf_at_confirm_Lph": inc.get("mnf_at_confirm_Lph"),
            "avg_mnf_Lph": inc.get("avg_mnf_Lph"),
            "volume_kL": inc.get("volume_lost_kL"),
    ...
    # -------------------------
    # Callback: Toggle recurrence collapse in Ignore Modal
    # -------------------------
    @app.callback(
        Output("modal-ignore-recurrence-collapse", "is_open"),
        Input("modal-ignore-is-recurring", "value"),
        prevent_initial_call=True,
    )
    def toggle_recurrence_collapse(is_recurring):
        return is_recurring

    # -------------------------
    # Callback: Show/hide auto-suppress info
    # -------------------------
    @app.callback(
        Output("modal-ignore-auto-suppress-info", "style"),
        Input("modal-ignore-auto-suppress", "value"),
        prevent_initial_call=True,
    )
    def toggle_auto_suppress_info(auto_suppress):
        if auto_suppress:
            return {"display": "block"}
        return {"display": "none"}

    # -------------------------
    # Callback: Show/hide days container based on recurrence type
    # -------------------------
    @app.callback(
        Output("modal-ignore-days-container", "style"),
        Input("modal-ignore-recurrence-type", "value"),
        prevent_initial_call=True,
    )
    def toggle_days_container(recurrence_type):
        if recurrence_type == "weekly":
            return {"display": "block"}
        return {"display": "none"}

    # -------------------------
    # Callback: Pre-fill recurrence days based on current incident
    # -------------------------
    @app.callback(
        Output("modal-ignore-recurrence-days", "value"),
        Input("modal-ignore", "is_open"),
        State("store-action-context", "data"),
        prevent_initial_call=True,
    )
    def prefill_recurrence_days(is_open, context):
        if not is_open or not context:
            return []
        ...
        try:
            ...
            if start_day:
                ...
                return [dt.dayofweek]  # Pre-select the day of week of the incident
        except Exception:
            pass
        return []
    ...
    @app.callback(
        ...
        [
            ...
            Input("modal-ignore-confirm-with-pattern", "n_clicks"),
        ...
        [
            ...
            State("modal-ignore-is-recurring", "value"),
            State("modal-ignore-recurrence-type", "value"),
            State("modal-ignore-recurrence-days", "value"),
            State("modal-ignore-time-start", "value"),
            State("modal-ignore-time-end", "value"),
            State("modal-ignore-auto-suppress", "value"),
        ],
        prevent_initial_call=True,
    )
    def confirm_ignore(
        ...
        # Determine if we should record a pattern
        should_record_pattern = ctx.triggered_id == "modal-ignore-confirm-with-pattern"

        # Save the action to action log (with reason - BUG FIX)
        df = append_action_row(
            context["site_id"],
            context["event_id"],
            context["start_day"],
            ...
            notes=notes,
            reason=reason,
            resolution_type=get_category_display_name(reason) if reason else None,
        )

        pattern_message = ""

        # Record pattern if requested
        if should_record_pattern:
            # Build incident dict from context for pattern recording
            incident_data = {
                "site_id": context.get("site_id"),
                "event_id": context.get("event_id"),
                "start_day": context.get("start_day"),
                "end_day": context.get("end_day"),
                "subscores_ui": context.get("subscores_ui", {}),
                "subscores": context.get("subscores", {}),
                "mnf_at_confirm_Lph": context.get("mnf_at_confirm_Lph"),
                "avg_mnf_Lph": context.get("avg_mnf_Lph"),
                "volume_kL": context.get("volume_kL"),
                "duration_hours": context.get("duration_hours"),
            }

            # Build description from reason and notes
            description = get_category_display_name(reason) if reason else "Unknown"
            if notes:
            ...
            result = record_pattern(
                ...
                time_window_start=time_start if is_recurring else None,
                time_window_end=time_end if is_recurring else None,
                auto_suppress=auto_suppress or False,
                notes=notes or "",
                user="dashboard_user",
            )
            ...
            if result.get("success"):
                pattern_message = (
                    f" | Pattern {result['action']}: {result['pattern_id']}"
                )
                log.info(f"Pattern recorded: {result}")

        # Build toast message
        toast_text = f"🚫 Ignored {context['event_id']}"
        if pattern_message:
            toast_text += pattern_message

        if auto_suppress:
            toast_text += " | Auto-suppress enabled"

        toast = dbc.Alert(
            toast_text,
            color="info" if should_record_pattern else "dark",
            duration=5000 if should_record_pattern else 3000,
            is_open=True,
        )
    ...
    # -------------------------
    # Callback: Display Recorded Patterns Table
    # -------------------------
    @app.callback(
        Output("patterns-table-container", "children"),
        [
            Input("btn-refresh-patterns", "n_clicks"),
            Input("action-log-refresh", "children"),
        ],
        prevent_initial_call=False,
    )
    def display_patterns_table(n_clicks, refresh_trigger):
        """Display the recorded false alarm patterns in a table."""
        from false_alarm_patterns import get_all_patterns, get_category_display_name
        ...
        if not patterns:
            return dbc.Alert(
                [
                    ...
                    "No patterns recorded yet. When you mark an event as 'Ignore' and click ",
                    html.Strong("'🧠 Ignore & Learn Pattern'"),
                    ", the pattern will appear here.",
                ],
                color="info",
                className="mb-0",
            )
        ...
        for p in patterns:
            ...
            elif p.get("auto_suppress"):
                status_color = "#F59E0B"
                status_text = "Auto-suppress"
            else:
                status_color = "#22C55E"
                status_text = "Active"
            ...
            recurrence_info = ""
            rec_rule = p.get("recurrence_rule", {})
            if isinstance(rec_rule, dict) and rec_rule.get("is_recurring"):
                rec_type = rec_rule.get("type", "")
                days = rec_rule.get("days_of_week", [])
                if days:
                    day_names = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]
                    day_str = ", ".join([day_names[d] for d in days if d < 7])
                    recurrence_info = f"{rec_type.title()} ({day_str})"
                else:
                    recurrence_info = rec_type.title() if rec_type else "Recurring"
                        ...
                        [
                            ...
                            html.Div(
                                [
                                    ...
                                    html.Span(
                                        f" - {p.get('description', '')[:50]}{'...' if len(p.get('description', '')) > 50 else ''}",
                                        style={
                                            "color": "#A1A1AA",
                                            "fontSize": "0.85rem",
                                        },
                            ...
                            html.Div(
                                [
                                    ...
                                    html.Small(
                                        f"Matched: {p.get('times_matched', 0)}x",
                                        className="me-3",
                                        style={"color": "#71717A"},
                                    ),
                                    html.Small(
                                        f"Confirmed: {p.get('times_confirmed_false', 0)}x",
                                        className="me-3",
                                        style={"color": "#22C55E"},
                                    ),
                                    html.Small(
                                        f"Was Real: {p.get('times_was_real_leak', 0)}x",
                                        style={"color": "#EF4444"},
                                    ),
                                ],
                                ...
                                [
                                    ...
                                    html.Small(
                                        ...
                                        style={
                                            ...
                                            "fontWeight": (
                                                "500" if recurrence_info else "400"
                                            ),
                                        },
                                    ),
                                ],
        ...
        return html.Div(
            [
                html.Div(
                    f"📊 {len(patterns)} pattern(s) recorded",
                    style={
                        "color": "#A1A1AA",
                        "fontSize": "0.85rem",
                        "marginBottom": "12px",
